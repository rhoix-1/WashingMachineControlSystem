...
def sendData():
	...
	# --- LOGS ---
	sql = "INSERT INTO logs (DateTime, AgitationWaterAmount_ml, AgitationWaterSalvaged_ml, SoakWaterAmount_ml, SoakWaterSalvaged_ml, Lights, Valves, Motors, Pumps, HumiditySensors, TemperatureSensors, PressureSensors) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
	logAWA = AgitationWaterAmount()
	logAWS = AgitationWaterSalvaged()
	logSWA = SoakWaterAmount()
	logSWS = SoakWaterSalvaged()
	logLights = Lights()
	logValves = Valves()
	logMotors = Motors()
	logPumps = Pumps()
	...
	val = (Now(), logAWA, logAWS, logSWA, logSWS, logLights, logValves, logMotors, logPumps, logHSensors, logTSensors, logPSensors)
	print(sql, val)
	...
	# --- SETINGS ---
	sql = "INSERT INTO settings (DateTime, ItemOfClothing, SizeOfItem, AgitationSpeed, CycleDuration, Soak) VALUES (%s, %s, %s, %s, %s, %s)"
	val = (Now(), setItemOfClothing, setSizeOfItem, setASpeed, setCDuration, setSoak)
	print(sql, val)
	mycursor.execute(sql, val)
...
class HomeWindow(Screen):
	...
	def doneButton(self, presetsID, agitationSpeedID, setCDurationID, soakItemID, sizeOfItemID):
		global setItemOfClothing
		global setASpeed
		global setCDuration
		global setSoak
		global setSizeOfItem
		setItemOfClothing = presetsID.text
		setASpeed = agitationSpeedID.text
		setCDuration = setCDurationID.text
		setSoak = soakItemID.text
		setSizeOfItem = sizeOfItemID.text
		sendData()
