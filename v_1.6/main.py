...
class Home_Window(tk.Frame):
	def __init__(self, parent, controller):
			...
			if progress_thread_running == False:
				...
				if intake_nump < 100:
					...
					if intake_nump == 0:
						...
						itemIntakeLoaded, dumby_0 = data.distanceFunc()
					...
					if agitation_nump == 0:
						...
						waterValveOpened, dumby_1 = data.servoFunc()
						...
						dumby_2, agitationMotorSpeedRPM, dumby_3 = data.motorFunc(1, agitationSpeedRPM, agitationDurationMIN)
					...
					if cycle_nump == 0:
						...
						dumby_4, cycleMotorSpeedRPM, dumby_5 = data.motorFunc(-1, cycleSpeedRPM, cycleDurationMIN)
						cycleMotorSpeedRPM = int(cycleMotorSpeedRPM)
					# Get data at ending
					elif cycle_nump == 90:
						dumby_6, dumby_7, humidityValue, temperatureValueC = data.dhtFunc()
					...
					if uv_light_nump == 0:
						...
						uvSelfCheckActive, uvLDRCheckActive, uvBrightness = data.lightFunc()
					...
					if dispense_nump == 0:
						...
						itemDispenseUnloaded, dumby_8 = data.distanceFunc()
		...
		home_date_and_time_text.grid(row=0, column=2, columnspan=3, padx=(56, 50))
...
class Settings_Window(tk.Frame):
	def __init__(self, parent, controller):
		...
		settings_date_and_time_text.grid(row=0, column=1, columnspan=3, padx=(110, 50))
